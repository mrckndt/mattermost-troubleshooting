...
argument-hint: [<name> | add <name> [<repos>] | remove <name>]
---
...
Behavior depends on the argument. All mutations edit `graphs/config.json` in place. A bundle definition carries only a `repos` array; a bundle with no `repos` is skipped during merge (`/graphify-update`, `/bootstrap` cascade). Keyword-based scope routing is driven by per-repo `keywords` arrays elsewhere in `graphs/config.json`, not by bundles.
...
3. Report a Markdown table with columns `Bundle | Repos | Built`.
   - `Repos`: comma-separated values, or `—` if missing/empty.
   - `Built`: `yes` or `no`.
...
2. Print the bundle's `repos` as a bullet list (`(none)` when empty).
...
## `add <name> [<repos>]` argument: create a bundle
...
3. Build the bundle object: `{"repos": [...]}`. Omit the `repos` key if the resolved list is empty.
...
- `repos` is optional. Behaviour with missing values: empty `repos` -> bundle skipped during merge.
- Bundles no longer carry `keywords`. Keyword-based scope routing is driven by per-repo `keywords` in `graphs/config.json#/repos/<repo>/keywords`. To make a bundle reachable via auto-select, add appropriate keywords to one of its member repos.
- To modify an existing bundle's repos, edit `graphs/config.json` directly, or `remove` and re-`add` the bundle. The slash command is intentionally narrow (list/add/remove); finer-grained edits are short and clear in the JSON.
