---
description: Build graphify knowledge graphs for one or more repos. Run after /bootstrap to create graphs for the first time.
argument-hint: [<bundle-name> | all | <repo>]
---
...
Source `.claude/secrets.env` if present so Python subprocesses inherit project-scoped API keys. If neither `GEMINI_API_KEY` nor `GOOGLE_API_KEY` is set after sourcing, print the Gemini tip:
...
No-op if `.claude/secrets.env` is absent. Each Bash tool call is a fresh subshell - env vars do not persist. Every Bash call that invokes `graphify.llm.extract_corpus_parallel` must re-source `.claude/secrets.env` in the same call: `[ -f .claude/secrets.env ] && . .claude/secrets.env;`. (`secrets.env` uses `export` lines, so plain `.` sourcing is sufficient.)

**Prereq checks**: `command -v graphify` (if missing, print `graphify CLI not installed - see README.md for install instructions. Skipping graph build.` and stop) and `[ -f graphs/config.json ]` (if missing, print `graphs/config.json not found. Skipping graph build.` and stop).

Args: $ARGUMENTS

Parse `$ARGUMENTS` for a selector. Valid selectors:
...
If no argument is given, read `graphs/config.json#/bundles`, then prompt: `Build graphify graphs? [<bundle1>|<bundle2>|all|<repo>]`. Wait for the answer.

**Resolve build set**: read `graphs/config.json`. For `<bundle-name>`, take `bundles.<bundle-name>.repos`. For `all`, take all keys under `repos`. For `<repo>`, take just that one. Skip any repo not present under `upstream/<repo>/` with a warning row in the report.

**For each repo in the build set** (parallel Bash tool calls where independent; do not chain with `&&` or pipes):
   - Always rebuilds: any existing `graphs/<repo>/graphify-out/graph.json` is overwritten by the new pipeline run. For a cheap code-only refresh that preserves the existing graph, use `/graphify-update <repo>` instead.
...
- If one repo fails, continue and report at the end. Retry with `/graphify-build <repo>`.
- `/graphify-build <repo>` always rebuilds from scratch (full pipeline: AST + semantic extract + cluster + label). For a cheap code-only refresh of an existing graph, use `/graphify-update <repo>` instead.
